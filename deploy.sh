...
# PATH normalization
# ============================================================================
# deploy.sh 実行時に bootstrap.sh でインストールされたツール
# (uv, sheldon 等) が PATH 未登録の状態でも検出できるよう、
# 必要最低限のパスを先頭に補強する
#   - ${HOME}/.local/bin : uv, sheldon 等のユーザーローカルインストール先
#   - /usr/local/bin     : 手動ビルド/インストールされたツール
#   - /usr/bin           : ディストリ標準パッケージ
# 既存 PATH は維持しつつ前置するため、ユーザー設定を上書きしない
...
setup_nvim_python_env() {
  ...
  uv pip install --python "$venv_python" pynvim >/dev/null 2>&1
